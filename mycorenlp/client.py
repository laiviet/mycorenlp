    ...
    def __init__(self, start_server=True,
        ...
        if start_server:
            ...
            assert os.getenv("CORENLP_HOME") is not None, \
                "Please define $CORENLP_HOME where your CoreNLP Java checkout is"
            print('CORENLP_HOME', os.getenv("CORENLP_HOME"))
            start_cmd = "java -cp '{corenlp_home}/*'  edu.stanford.nlp.pipeline.StanfordCoreNLPServer -port {port} -timeout {timeout} -threads {thread}".format(
